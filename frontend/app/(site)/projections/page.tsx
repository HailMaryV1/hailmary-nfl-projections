...
// The real, dedicated "every player, every week" table - what the
...
export default async function ProjectionsPage({ searchParams }: { searchParams: Promise<{ horizon?: string }> }) {
  ...
  return (
    <main className="mx-auto w-full min-w-0 max-w-5xl flex-1 p-4 sm:p-6">
        ...
        <h1 className="text-2xl font-semibold text-navy-100">Player Projections</h1>
        <p className="mt-1 max-w-2xl text-sm text-navy-300">
          Every real active FanTeam NFL player, priced with a real projected points total for this horizon - five layers deep, sortable by points or real
          value per £m.
        </p>
        ...
        {players.length === 0 ? (
          ...
          <div className="mt-6">
            <RatingsTable players={players} horizon={horizon} defaultSortMode="points" />
          </div>
        )}
    </main>
  );
